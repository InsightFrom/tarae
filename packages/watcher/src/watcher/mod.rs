...
pub fn start_background_watcher(server: TaraeServer) {
    tokio::spawn(async move {
        // 1. Resolve a safe project root. Never watch arbitrary MCP client cwd.
        let project_root = match root::resolve_project_root(server.config.project_root.as_deref()) {
            Ok(Some(root)) => root,
            Ok(None) => {
                warn!(
                    "Background file watcher disabled: no safe project root detected. Set TARAE_PROJECT_ROOT or run topa from a git/project directory."
                );
                return;
            }
            Err(e) => {
                warn!("Background file watcher disabled: {}", e);
                return;
            }
        };
            ...
            tokio::select! {
                ...
                res = handle.rx.recv() => {
                    match res {
                        Some(changes) => {
                            ...
                            let is_mcp_active = server.is_active_session_running().await;
                            ...
                            if is_ai_active {
                                ...
                                if auto_checkpoint.record(&changes) {
                                    ...
                                    let event = server.create_event(EventType::AutoCheckpoint, payload).await;
                                    let msg = server.record_event(&event).await;
                ...
                _ = &mut timer_fut, if has_timer => {
                    ...
                    if !pending_human_changes.is_empty() {
                        ...
                        let mut event = server.create_event(EventType::HumanIntervention, payload).await;
                        event.actor = Actor {
